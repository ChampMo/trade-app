...
- [ ] P0-08 Broker time offset: read MT5 server time vs UTC on connect, store `server_utc_offset_min` with every order/event that carries broker time; document the tick-time caveat. Done when: `Journal.event` rows from the bridge carry the offset and a unit test covers the conversion.
