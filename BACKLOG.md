...
- [ ] P1-05b Wire the kill switch to the things that feed it. Half done 2026-09-03: the executor now produces `consecutive_rejects` and `last_broker_contact_utc`, and a test drives three rejections through to the kill trigger. Still missing: `reconcile_mismatch` from P1-08, `positions_without_stop` from a periodic stop audit, and a loop that calls `check_and_trip` on a timer (needs P1-09/P1-10). Done when: the core loop calls it every tick and a test drives a full RUNNING → KILLED → PAUSED → RUNNING cycle.
