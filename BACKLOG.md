...
- [ ] P1-05b Wire the kill switch to the things that feed it: `last_broker_contact_utc` from the bridge, `consecutive_rejects` from the execution layer (P1-06), `reconcile_mismatch` from P1-08, `positions_without_stop` from a periodic stop audit. Until then the triggers are correct but nothing calls `check_and_trip` on a timer. Done when: the core loop calls it every tick and a test drives a full RUNNING → KILLED → PAUSED → RUNNING cycle.
