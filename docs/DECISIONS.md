...
## คำถามที่ยังต้องตัดสินใจ

- **Kill switch ปิดไม้ที่คนเปิดเองด้วยไหม** ตอนนี้ `KillSwitch` ปิดทุกโพซิชันในบัญชี รวมถึงไม้ที่เปิดด้วยมือจาก terminal ส่วน `Reconciler` แยกออกได้ว่าไม้ไหนเป็นของบอท (ดูจาก magic number) สองตัวนี้จึงมองไม่ตรงกัน · ถ้าบัญชีนี้ใช้กับบอทอย่างเดียวตามที่ตั้งใจ (D8) ก็ไม่มีปัญหาและ "ปิดให้หมด" คือทางที่ปลอดภัยกว่าสำหรับเงินทุน · ถ้าคุณจะเทรดมือบนบัญชีเดียวกันด้วย ต้องเปลี่ยนให้ kill switch ปิดเฉพาะ magic ของบอท **ยังไม่ได้ตัดสินใจ รอเจ้าของ**
